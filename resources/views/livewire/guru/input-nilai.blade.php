<div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @forelse ($daftarKelompok as $kelompok)
                <button wire:click="selectKelompok({{ $kelompok->id_kelompok }})" 
                        class="p-4 bg-gray-50 rounded-lg shadow border border-gray-200 hover:bg-green-100 hover:border-green-400 transition-all">
                    <p class="text-xl font-semibold text-gray-800">{{ $kelompok->kelas->nama_kelas ?? 'Tanpa Kelas' }}</p>
                    <p class="text-sm text-gray-500">{{ $kelompok->tahun_ajaran }}</p>
                </button>
            @empty
                <p class="text-gray-500 col-span-3">Data kelompok tidak ditemukan. (Pastikan Anda sudah menjalankan Seeder)</p>
            @endforelse
        </div>
    </div>
    @endif
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="flex justify-between items-center border-b pb-2 mb-4">
            ...
            <button wire:click="backStep(1)" class="text-sm text-gray-500 hover:text-gray-800">&larr; Kembali ke Kelompok</button>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @forelse ($daftarSiswa as $siswa)
                <button wire:click="selectSiswa({{ $siswa->id_siswa }})" 
                    ...
                    <p class="text-sm text-gray-500">{{ $siswa->kelas->nama_kelas ?? 'Tanpa Kelas' }}</p>
                </button>
            @empty
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="flex justify-between items-center border-b pb-2 mb-4">
            ...
            <button wire:click="backStep(2)" class="text-sm text-gray-500 hover:text-gray-800">&larr; Kembali ke Siswa</button>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-1">
                    ...
                    <select wire:model.live="id_surah" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        ...
                        <option value="{{ $surah->id_surah }}">{{ $surah->nomor_surah }}. {{ $surah->nama_surah }}</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-1">
                    ...
                    <input wire:model="ayat_mulai" type="number" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                ...
                <div class="md:col-span-1">
                    ...
                    <input wire:model="ayat_selesai" type="number" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    @error('ayat_selesai') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
            </div>
            
            <div class="mt-6 text-right">
                <button type="submit" wire:loading.attr="disabled" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-6 rounded-lg">
                    Mulai Menilai &rarr;
                </button>
            </div>
        </form>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="p-4 border rounded-lg bg-gray-50 mb-6" style="direction: rtl; font-family: 'Times New Roman', Times, serif; font-size: 24px; line-height: 1.75;">
                ...
                <p class="mb-4">
                    ...
                    @foreach($words as $index => $word)
                        @php
                            ...
                            $bgColor = ''; // Default
                            if ($koreksiKata) {
                                if ($koreksiKata['kategori'] == 'tajwid') $bgColor = 'bg-red-200';
                                elseif ($koreksiKata['kategori'] == 'makhroj') $bgColor = 'bg-blue-200';
                                elseif ($koreksiKata['kategori'] == 'kelancaran') $bgColor = 'bg-yellow-200';
                            }
                        @endphp
                        
                        <span x-data="{ open: false }" class="relative inline-block p-1 rounded cursor-pointer {{ $bgColor }} hover:bg-gray-200">
                            ...
                            <span x-show="open" @click.away="open = false" 
                                ...
                                <button @click="open = false" wire:click="addKoreksi({{ $ayat->id_ayat }}, {{ $index }}, 'tajwid', '{{ $word }}')" 
                                        class="px-2 py-1 bg-red-100 hover:bg-red-300 rounded text-xs font-bold text-red-800">Tajwid</button>
                                
                                <button @click="open = false" wire:click="addKoreksi({{ $ayat->id_ayat }}, {{ $index }}, 'makhroj', '{{ $word }}')" 
                                        class="px-2 py-1 bg-blue-100 hover:bg-blue-300 rounded text-xs font-bold text-blue-800">Makhroj</button>
                                
                                <button @click="open = false" wire:click="addKoreksi({{ $ayat->id_ayat }}, {{ $index }}, 'kelancaran', '{{ $word }}')" 
        ...
        <div class="bg-gray-50 p-6 rounded-lg shadow-inner mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                <div>
                    ...
                    <p class="text-sm text-gray-600">Total Kata: <span class="font-bold">{{ $this->statistik['totalKata'] }}</span></an>
                    <ul class="mt-2 space-y-1 text-sm">
                        <li>Tajwid: <span class="font-bold">{{ $this->statistik['totalKesalahanTajwid'] }} / {{ $this->statistik['totalKata'] }}</span> ({{ $this->statistik['proporsiTajwid'] }}%)</li>
                        <li>Makhroj: <span class="font-bold">{{ $this->statistik['totalKesalahanMakhroj'] }} / {{ $this->statistik['totalKata'] }}</span> ({{ $this->statistik['proporsiMakhroj'] }}%)</li>
                        <li>Kelancaran: <span class="font-bold">{{ $this->statistik['totalKesalahanKelancaran'] }} / {{ $this->statistik['totalKata'] }}</span> ({{ $this->statistik['proporsiKelancaran'] }}%)</li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="mt-2 space-y-1 text-sm">
                        <li>Skor Tajwid: <span class="font-bold text-blue-600">{{ $this->statistik['skorTajwid'] }}</span></li>
                        <li>Skor Makhroj: <span class="font-bold text-blue-600">{{ $this->statistik['skorMakhroj'] }}</span></li>
                        <li>Skor Kelancaran: <span class="font-bold text-blue-600">{{ $this->statistik['skorKelancaran'] }}</span></li>
                    </ul>
                    <hr class="my-2">
