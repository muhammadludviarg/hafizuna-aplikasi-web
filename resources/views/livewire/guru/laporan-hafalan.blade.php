<div>
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-4 md:p-6 text-gray-900 dark:text-gray-100">
            ...
            {{-- 1. TAMPILAN GRID PILIH KELOMPOK --}}
            @if (!$selectedKelompokId)
                <div wire:key="view-kelompok">
                    ...
                    <p class="text-sm md:text-base text-gray-600 mb-6">Pilih kelompok untuk melihat laporan siswa</p>

                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        @forelse ($kelompokList as $kelompok)
                            <div wire:click="selectKelompok({{ $kelompok['id'] }})"
                                class="bg-white border-2 border-gray-200 rounded-lg p-5 md:p-6 cursor-pointer hover:shadow-lg hover:border-green-500 transition-all duration-200 active:bg-green-50 flex flex-col justify-between h-full">
            ...
            {{-- 2. TAMPILAN DAFTAR SISWA DALAM KELOMPOK --}}
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                        <div>
                            ...
                            <h3 class="text-xl md:text-2xl font-bold text-gray-800">
                                {{ $kelompokDetail['nama_kelompok'] ?? 'Detail Laporan' }}
                            </h3>
                            <p class="text-sm text-gray-600 mt-1">
                                Tahun Ajaran: {{ $kelompokDetail['tahun_ajaran'] ?? 'N/A' }}
                            </p>
                        </div>
            ...
            {{-- 3. TAMPILAN DETAIL SISWA --}}
            @elseif ($selectedSiswaId && !$selectedSurahId)
                {{-- Bagian ini SAMA PERSIS dengan Admin UI, karena struktur datanya sudah disamakan di Controller --}}
                <div wire:key="view-detail-siswa">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                        {{-- Perhatikan method ini: backToKelas (sesuai Controller Guru) --}}
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4 mb-6">
                        <div class="col-span-2 lg:col-span-1 border-2 border-green-600 rounded-lg p-4 bg-green-50">
                            <p class="text-green-800 text-xs md:text-sm mb-1 uppercase tracking-wide font-semibold">Total Sesi</p>
                            <p class="text-2xl md:text-3xl font-bold text-green-700">{{ $siswaDetail['jumlah_sesi'] ?? 0 }}</p>
                        </div>
                        <div class="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
                            <p class="text-gray-500 text-xs md:text-sm mb-1">Tajwid</p>
                            <p class="text-xl md:text-2xl font-bold text-gray-800">{{ $siswaDetail['nilai_tajwid'] ?? 0 }}</p>
                        </div>
                        <div class="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
                        ...
                        <div class="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
                            ...
                            <p class="text-xl md:text-2xl font-bold text-gray-800">{{ $siswaDetail['nilai_makhroj'] ?? 0 }}</p>
                        </div>
                            ...
                            @forelse ($siswaDetail['riwayat_sesi'] as $index => $sesi)
                                <div wire:click="selectSurah({{ $sesi['id_surah_mulai'] }})"
                                    ...
                                    <div class="flex flex-col sm:flex-row gap-4">
                                        ...
                                        <div class="grid grid-cols-4 gap-2 sm:gap-4 border-t sm:border-t-0 sm:border-l border-gray-100 pt-3 sm:pt-0 sm:pl-4">
                        ...
                        <div class="w-full overflow-x-auto rounded-lg border border-gray-200">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-orange-50">
                                    <tr>
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-orange-800 uppercase tracking-wider whitespace-nowrap">No</th>
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-orange-800 uppercase tracking-wider whitespace-nowrap">Surah</th>
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-orange-800 uppercase tracking-wider whitespace-nowrap">Status</th>
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-orange-800 uppercase tracking-wider whitespace-nowrap">Progress</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
            ...
            @else
                <div wire:key="view-detail-surah">
                    ...
                    <div class="bg-white border-l-4 border-green-600 rounded-r-lg shadow-sm p-4 mb-6 flex items-center justify-between">
                        <div>
                        ...
                        <div class="bg-green-100 text-green-800 rounded-full w-12 h-12 flex items-center justify-center font-bold text-xl border border-green-200">
                                ...
                                <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                                    <div class="flex items-center justify-between">
                                        <div>
                                            ...
                                            <div class="inline-flex items-center mt-1 px-2 py-0.5 rounded text-xs bg-gray-100 text-gray-600">
                                                Ayat {{ $sesi['ayat_mulai'] }}-{{ $sesi['ayat_selesai'] }}
                                            </div>
                                        </div>
                    ...
                    {{-- MODAL DETAIL SESI / KOREKSI --}}
                    @if(isset($selectedSesiDetail) && $selectedSesiDetail)
                        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                            <div class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity backdrop-blur-sm" wire:click="closeSesiDetail"></div>

                            <div class="flex min-h-full items-end justify-center p-0 md:p-4 text-center sm:items-center sm:p-0">
                                <div class="relative transform overflow-hidden rounded-t-2xl md:rounded-2xl bg-white text-left shadow-xl transition-all w-full sm:max-w-2xl md:max-w-4xl max-h-[90vh] flex flex-col">
                                    ...
                                    <div class="bg-green-600 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shrink-0">
                                        ...
                                        <button wire:click="closeSesiDetail" class="text-green-100 hover:text-white transition-colors">
                                    ...
                                    <div class="p-4 md:p-6 overflow-y-auto">
                                        ...
                                        <div class="grid grid-cols-4 gap-2 mb-6">
                                            <div class="text-center p-2 bg-gray-50 rounded">
                                                ...
                                                <span class="block text-lg font-bold text-gray-800">{{ $selectedSesiDetail['nilai_tajwid'] }}</span>
                                            </div>
                                            <div class="text-center p-2 bg-gray-50 rounded">
                                                <span class="block text-[10px] text-gray-500 uppercase">Lancar</span>
                                                <span class="block text-lg font-bold text-gray-800">{{ $selectedSesiDetail['nilai_kelancaran'] }}</span>
                                            </div>
                                            <div class="text-center p-2 bg-gray-50 rounded">
                                                <span class="block text-[10px] text-gray-500 uppercase">Makhroj</span>
                                                <span class="block text-lg font-bold text-gray-800">{{ $selectedSesiDetail['nilai_makhroj'] }}</span>
                                            </div>
                                            <div class="text-center p-2 bg-green-100 rounded border border-green-200">
                                                ...
                                                <span class="block text-lg font-bold text-green-800">{{ $selectedSesiDetail['nilai_rata'] }}</span>
                                            </div>
                                        </div>
                                                    ...
                                                    <table class="min-w-full divide-y divide-gray-200">
                                                        <thead class="bg-gray-50">
                                                            <tr>
                                                                ...
                                                                <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Lafadz</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody class="bg-white divide-y divide-gray-200">
                                                            @forelse($selectedSesiDetail['koreksi'] as $kor)
                                                                <tr>
                                                                    <td class="px-3 py-3 text-sm text-gray-800 font-medium">{{ $kor['lokasi'] }}</td>
                                                                    <td class="px-3 py-3 text-sm text-red-600">{{ $kor['jenis_kesalahan'] }}</td>
                                                                    <td class="px-3 py-3 text-sm text-right font-arabic" style="font-family: 'Amiri', serif; font-size: 1.2em;">
                                                                        {{ $kor['catatan'] }}
                                                                    </td>
                                                                </tr>
                                                            @empty
                                                                <tr>
                                                                    <td colspan="3" class="px-4 py-6 text-center text-sm text-gray-500 italic bg-gray-50">
                                    ...
                                    <div class="bg-gray-50 px-4 py-3 sm:px-6 flex flex-row-reverse border-t border-gray-100 shrink-0">
