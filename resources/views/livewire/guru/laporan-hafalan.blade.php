    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-4 md:p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        @forelse ($kelompokList as $kelompok)
                            <div wire:click="selectKelompok({{ $kelompok['id'] }})"
                                class="group bg-white border-2 border-gray-200 rounded-xl p-5 md:p-6 cursor-pointer hover:shadow-xl hover:shadow-green-100 hover:border-green-500 transition-all duration-300 transform hover:-translate-y-1 active:scale-95 flex flex-col justify-between h-full">

                                <div>
                                    {{-- Label --}}
                                    <div class="flex items-start justify-between mb-3">
                                        <span
                                            class="bg-green-100 text-green-800 text-xs font-bold px-2.5 py-0.5 rounded border border-green-200">Kelompok</span>
                                    </div>
                                    {{-- Nama Kelompok --}}
                                    <h4
                                        class="text-lg font-bold text-gray-800 mb-1 group-hover:text-green-700 transition-colors">
                                        {{ $kelompok['nama_kelompok_utama'] }}
                                    </h4>
                                    ...
                                    <div class="flex items-center gap-1.5 text-gray-500 mb-4">
                                        ...
                                        <p class="text-sm font-medium">{{ $kelompok['nama_kelas_kecil'] }}</p>
                                    </div>
                                </div>

                                <div class="flex items-center justify-between border-t border-gray-100 pt-4">
                                    <span
                                        class="text-xs font-bold text-gray-700 bg-gray-100 px-3 py-1 rounded-full group-hover:bg-green-600 group-hover:text-white transition-colors">
                        ...
                        @empty
                            <div
                                ...
                                <p class="text-gray-500">Anda belum memiliki kelompok hafalan.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
            ...
            @elseif (!$selectedSiswaId && !$selectedSurahId)
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($detailLaporan as $index => $siswa)
                                    <tr wire:click="selectSiswa({{ $siswa['id_siswa'] }})"
                                        ...
                                        <td
                                            class="px-4 md:px-6 py-4 text-sm font-bold text-gray-800 group-hover:text-green-700">
                                            {{ $siswa['nama_siswa'] }}
                                        </td>
                ...
                {{-- ========================================================================
                BAGIAN 3: DETAIL SISWA (STATS, HISTORY, SURAH)
                ======================================================================== --}}
            @elseif ($selectedSiswaId && !$selectedSurahId)
                <div wire:key="view-detail-siswa">
                        ...
                        <div class="flex flex-col md:flex-row gap-4 items-end">
                            ...
                            <div class="w-full md:w-auto">
                                <button wire:click="filterPeriode()"
                                    class="w-full md:w-auto px-6 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800 text-sm font-semibold transition-colors shadow-sm active:transform active:scale-95">
                                    Terapkan
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="mb-10">
                        <h4 class="text-xl font-bold text-gray-800 mb-5 flex items-center gap-3">
                            ...
                            <div>
                                Riwayat Sesi
                                <span class="block text-xs font-normal text-gray-500 mt-0.5">Total
                                ...
                                <div wire:click="selectSurah({{ $sesi['id_surah_mulai'] }})"
                                    ...
                                    <div class="flex flex-col sm:flex-row gap-4 items-center">
                                        <div class="flex items-start gap-4 flex-1 w-full">
                                            <span
                                                class="flex-shrink-0 bg-green-50 text-green-700 border border-green-100 rounded-lg w-12 h-12 flex items-center justify-center font-bold text-lg group-hover:bg-green-600 group-hover:text-white transition-colors">
                                            ...
                                            <div>
                                                ...
                                                <div class="flex flex-wrap gap-2 mt-1">
                                                    <span
                                                        class="text-xs font-semibold text-gray-600 bg-gray-100 px-2 py-1 rounded border border-gray-200">
                        ...
                        @if($totalPagesSesi > 1)
                            <div
                                ...
                                <div class="text-sm text-gray-600 font-medium text-center sm:text-left">
                                    Halaman <span class="text-gray-900 font-bold">{{ $currentPageSesi }}</span> dari
                                    {{ $totalPagesSesi }}
                                </div>
