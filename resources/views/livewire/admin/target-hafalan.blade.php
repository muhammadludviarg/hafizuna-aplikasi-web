            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select wire:model="id_kelompok" 
                            {{ !$id_kelas ? 'disabled' : '' }}
                            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 {{ !$id_kelas ? 'bg-gray-100' : '' }}">
                        <option value="">{{ $id_kelas ? 'Pilih kelompok' : 'Pilih kelas dulu' }}</option>
                        @foreach($daftarKelompok as $kelompok)
                            <option value="{{ $kelompok->id_kelompok }}" {{ $id_kelompok == $kelompok->id_kelompok ? 'selected' : '' }}>
                                Kelompok {{ $kelompok->guru->akun->nama_lengkap ?? 'N/A' }}
                            </option>
                        @endforeach
                    </select>
